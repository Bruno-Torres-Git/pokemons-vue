<template>
    <v-app>
        ...
        <v-dialog v-model="show_dialog" width="800">
            <v-card v-if="selected_pokemon" class="px-2">
                <v-container>
                    ...
                    </v-row><!--Esto es un componente de vuetify que nos ayuda a hacer nuestra pagina responsive-->
                    <v-simple-table>
                        <template>
                            <thead>
                                <tr>
                                    <th class="text-left">Hability</th>
        ...
        </v-dialog><!--Use esto para que al hacer click nos aparezca el modal con las stats del pokemon-->
        <footer>Proyecto with PokeAPI | Bruno Torres</footer>
    </v-app>
</template>
<script>
import axios from "axios"//Esta es una promesa que nos va a servir para llamar y trabajar con las Apis
